// Node.js serverless function en Vercel (JavaScript)
...
export default async function handler(req, res) {
  ...
  const map = {
    TyCSportsAR: 'tyc-sports',
    DirectTvSports: 'directv-sports',
    TNTSportsAR: 'tnt-sports',
    TvPublicaAR: 'tv-publica',
    EspnPremiumAR: 'espn-premium',
    FoxSportsAR: 'fox-sports'
  };

  const slug = map[channelId];
  if (!slug) {
    return res.status(404).json({ error: `Unknown channelId "${channelId}"` });
  }
